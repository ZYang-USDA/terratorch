...
To work with temporal data, you need a compatible datamodule. For example, the generic TerraTorch datamodules support an `expand_temporal_dimension` argument, which reshapes inputs from `(T·C, H, W)` to `(C, T, H, W)` at load time.  
